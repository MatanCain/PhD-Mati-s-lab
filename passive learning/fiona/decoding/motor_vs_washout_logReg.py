...
plt.ylabel('% accuracy')
plt.title('logistic regression decoding population - motor block')
plt.ylim((0.3,1))
plt.grid()
plt.show()
...
for bin_inx,bin_begin in enumerate(bin_begins):
    ax[bin_inx].hist(weight_array_mean[:,bin_inx])
...
for bin_inx,bin_begin in enumerate(bin_begins):
    ...
    for inx in sorted_inxs:
        ...
        for cur_block in [motor_tasks,dishabituation_tasks]:
...
#     plt.plot(np.arange(1,81),FR_block) 
#     plt.plot(np.arange(1,81,2),FR_dis_pre) 
#     plt.plot(np.arange(1,81,2),FR_dis_post)
#     plt.title(cell_ID)
#     plt.legend(('motor','washout pre','washout post'))
#     plt.show()  