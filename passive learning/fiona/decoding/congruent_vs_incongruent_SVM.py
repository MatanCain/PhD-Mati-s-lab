...
for cell_ID in cell_list:
    ...
    if first_cong>first_incong:    
        ...
        FR_dis_post=cur_cell_task.get_mean_FR_event(dictFilterTrials_post,'motion_onset',window_pre=-500,window_post=0)    
        #concatenation
        FR_conc=np.concatenate((FR_dis_pre.to_numpy(),FR_cong.to_numpy(),FR_incong.to_numpy(),FR_dis_post.to_numpy()),axis=0)
...
n_iterations=100
kernel_svm='rbf'
c_svm=1 #should be 0 and 1. less than 1 for noisy data
...
temp_accuracy_array=np.empty([np.size(C_range),np.size(gamma_range),len(bin_begins)])
temp_accuracy_array[:]=np.nan
for c_inx,c in enumerate(C_range):
    for g_inx,gamma in enumerate(gamma_range):
        ...
        for bin_inx,bin_begin in enumerate(bin_begins):
            ...
            for ii in np.arange(n_iterations):
                ...
                clf=svm.SVC(kernel=kernel_svm,C=c_svm,gamma=gamma)
            ...
            temp_accuracy_array[c_inx,g_inx,bin_inx]=np.mean(accuracy_iterations_array)
        ...
        # plt.ylabel('% accuracy')
        # plt.title('SVM decoding population')
        # #plt.ylim((0.5,1))
        # plt.show()
...
# bin_begins=np.arange(-500,500,200)
# bin_length=200
# #paramaters for svm
# training_test_size=0.2 #in percentage
# n_iterations=100
