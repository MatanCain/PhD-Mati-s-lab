...
for cell_inx,cell_ID in enumerate(cur_cell_list):
    ...
    trials_motor_df_T0=cur_cell_task.filtTrials(dictFilterTrials_block)
...
# corr,pval=scipy.stats.wilcoxon(scatter_disPre_array,scatter_disNext_array)
# plt.scatter(scatter_disPre_array,scatter_disNext_array)
# plt.plot(x,x,color='red')
# plt.xlabel('dis pre')
# plt.ylabel('dis next')
# plt.title('dis Pre vs dis next - pval: '+ str(round(pval,5)))
# #plt.xlim((0,60))
# #plt.ylim((0,60))
# plt.show()
...
#compare the first trial of motor blocks to other
...
PSTH_block_array=np.zeros((len(cur_cell_list),np.size(time_course)))
PSTH_T0_block_array=np.zeros((len(cur_cell_list),np.size(time_course)))
...
for cell_inx,cell_ID in enumerate(cur_cell_list):
    for cur_task in motor_tasks:
        try:
            ...
            continue
        #whole motor block
        dictFilterTrials_block={'dir':'filterOff', 'trial_name':'filterOff', 'fail':0, 'after_fail':'filterOff','saccade_motion':'filterOff','blink_motion':'filterOff'} 
        PSTH=cur_cell_task.PSTH(window,dictFilterTrials_block,plot_option=0,smooth_option=1)
        PSTH=PSTH[filter_margin:-filter_margin]
        PSTH_block_array[cell_inx,:]=PSTH 
        scatter_motor_array.append(np.nanmean(PSTH[-(window['timeBefore']+filter_margin)+scatter_begin:-(window['timeBefore']+filter_margin)+scatter_end]))    

        #firs trial in motor block
        dictFilterTrials_block_T0={'dir':'filterOff', 'trial_name':'filterOff', 'fail':0, 'after_fail':'filterOff','saccade_motion':'filterOff','blink_motion':'filterOff','trial_begin_end':66} 
        PSTH=cur_cell_task.PSTH(window,dictFilterTrials_block_T0,plot_option=0,smooth_option=1)
        PSTH_T0=PSTH[filter_margin:-filter_margin]
        PSTH_T0_block_array[cell_inx,:]=PSTH_T0 
        scatter_motor_T0_array.append(np.nanmean(PSTH[-(window['timeBefore']+filter_margin)+scatter_begin:-(window['timeBefore']+filter_margin)+scatter_end]))    
        ...
        fb_pre=dis_pre_cells_pd.loc[dis_pre_cells_pd['cell_ID']==cell_ID,'file_begin'].item()
        fe_pre=dis_pre_cells_pd.loc[dis_pre_cells_pd['cell_ID']==cell_ID,'file_end'].item()
        dictFilterTrials_pre={'dir':'filterOff', 'trial_name':trial_type, 'fail':0, 'after_fail':'filterOff','saccade_motion':'filterOff','blink_motion':'filterOff','files_begin_end':[fb_pre,fe_pre],'trial_begin_end':'filterOff'}    
        PSTH=cur_cell_task.PSTH(window,dictFilterTrials_pre,plot_option=0,smooth_option=1) 
        PSTH=PSTH[filter_margin:-filter_margin]
        PSTH_disPre_array[cell_inx,:]=PSTH   
        ...
        fb_post=dis_next_cells_pd.loc[dis_next_cells_pd['cell_ID']==cell_ID,'file_begin'].item()
        fe_post=dis_next_cells_pd.loc[dis_next_cells_pd['cell_ID']==cell_ID,'file_end'].item()
        dictFilterTrials_post={'dir':'filterOff', 'trial_name':trial_type, 'fail':0, 'after_fail':'filterOff','saccade_motion':'filterOff','blink_motion':'filterOff','files_begin_end':[fb_post,fe_post],'trial_begin_end':'filterOff'}    
        PSTH=cur_cell_task.PSTH(window,dictFilterTrials_post,plot_option=0,smooth_option=1) 
        PSTH=PSTH[filter_margin:-filter_margin]
        PSTH_disNext_array[cell_inx,:]=PSTH 
        
PSTH_block=np.nanmean(PSTH_block_array,axis=0)
PSTH_block_T0=np.nanmean(PSTH_T0_block_array,axis=0)
...
plt.plot(time_course,PSTH_block,color='tab:blue')
plt.plot(time_course,PSTH_block_T0,color='tab:orange')
plt.plot(time_course,PSTH_disPre,color='lightgrey')
plt.plot(time_course,PSTH_disNext,color='lightgrey')
...
plt.xlabel('time from MO')
plt.ylabel('FR')
plt.legend(['block','T0','dis pre','dis next'])
plt.show()

# x=np.arange(0,100,1)
# plt.scatter(scatter_motor_array,scatter_motor_T0_array)
# plt.plot(x,x)
# plt.xlabel('motor')
# plt.ylabel('T0')
# plt.title(str(scatter_begin)+'-'+str(scatter_end))
# #plt.xlim((-1,50))
# #plt.ylim((-1,50))
# plt.show()
...
# PSTH_block=np.zeros((80,np.size(time_course)))
# PSTH_block_T0=np.zeros((80,np.size(time_course)))

# for trial_inx in np.arange(0,80):
#     print(trial_inx)
    
#     PSTH_block_trial_inx_array=np.zeros((len(cur_cell_list),np.size(time_course)))
#     PSTH_block_T0_trial_inx_array=np.zeros((len(cur_cell_list),np.size(time_course)))
...
#             PSTH=cur_cell_task.PSTH(window,dictFilterTrials_block,plot_option=0,smooth_option=1)
#             PSTH=PSTH[filter_margin:-filter_margin]
#             PSTH_block_trial_inx_array[cell_inx,:]=PSTH 
    
#             #firs trial in motor block
#             dictFilterTrials_block_T0={'dir':'filterOff', 'trial_name':'filterOff', 'fail':0, 'after_fail':'filterOff','saccade_motion':'filterOff','blink_motion':'filterOff','trial_begin_end':0} 
#             PSTH=cur_cell_task.PSTH(window,dictFilterTrials_block_T0,plot_option=0,smooth_option=1)
#             PSTH=PSTH[filter_margin:-filter_margin]
#             PSTH_block_T0_trial_inx_array[cell_inx,:]=PSTH 
    
            
#     PSTH_block_T0_trial_inx=np.nanmean(PSTH_block_T0_trial_inx_array,axis=0)
#     PSTH_block_T0[trial_inx,:]=PSTH_block_T0_trial_inx  

#     PSTH_block_trial_inx=np.nanmean(PSTH_block_trial_inx_array,axis=0)
#     PSTH_block[trial_inx,:]=PSTH_block_trial_inx  


# PSTH_block_mean=np.nanmean(PSTH_block,axis=0)
# PSTH_block_T0_mean=np.nanmean(PSTH_block_T0,axis=0)



# plt.plot(time_course,PSTH_block_mean)
# plt.plot(time_course,PSTH_block_T0_mean)
# #plt.axvline(x=100,color='black')
# #plt.axvline(x=300,color='black')
# plt.xlabel('time from MO')
# plt.ylabel('FR')
# #plt.legend(['average','trialByTrial'])
# plt.title('learned cells')
# plt.show()
