        ...
        for learning_task2_inx,learning_task2 in enumerate([learning_task+'_'+learned_direction,learning_task+'_'+learned_direction+'_100_25_cue']):       
            ...
            for cell_ID in cell_list:
                ...
                for block_inx,(begin_inx,end_inx) in enumerate(zip(block_begin_indexes,block_end_indexes)):
                    ...
                    dictFilterTrials_dishabituation = { 'dir':'filterOff', 'fail':0,'files_begin_end':[file_begin_dishabituation,file_end_dishabituation],'trial_name':'d0'+trial_type_learning,'even_odd_rows':'even'}
                    ...
                    try:
                        ...
                        dictFilterTrials_mapping_base_sign = {'dir':'filterOff', 'trial_name':trialname_base, 'fail':0}
                        dictFilterTrials_mapping_learned_sign = {'dir':'filterOff', 'trial_name':trialname_learned, 'fail':0}
                        FR_mapping_base_sign=cur_cell_mapping.get_mean_FR_event(dictFilterTrials_mapping_base_sign,'motion_onset',window_pre=window_begin,window_post=window_end)
                        FR_mapping_learned_sign=cur_cell_mapping.get_mean_FR_event(dictFilterTrials_mapping_learned_sign,'motion_onset',window_pre=window_begin,window_post=window_end)

                    except:
                        continue
                
                    block_dict=dict.fromkeys(['cell_ID','FR learning','FR dishabituation','PSTH learning','PSTH dishabituation','PSTH mapping base','PSTH mapping learned','FR mapping base','FR mapping learned','FR mapping base sign','FR mapping learned sign'])
                    ...
                    block_dict['FR mapping base sign']=FR_mapping_base_sign
                    block_dict['FR mapping learned sign']=FR_mapping_learned_sign
...
for array_inx,sig_array in enumerate([sig_cong_array,sig_incong_array]):
    ...
    for block_inx,cur_block_dict in enumerate(sig_array):
            ...
            mean_mapping_base=np.nanmean(cur_block_dict['FR mapping base sign'])
            mean_mapping_learned=np.nanmean(cur_block_dict['FR mapping learned sign'])
    ...
    stat,p_learned_base=stats.wilcoxon(learning_FR,dishabituation_FR)
    ...
    plt.legend(['dishabituation','learning'], loc ="lower right",fontsize=legend_size)
