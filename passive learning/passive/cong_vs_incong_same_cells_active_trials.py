...
cur_alternative='greater' # do not use two sided with subtract BL
subtract_BL=0 #subtract the bsaleine of each cell (-800:-400) from the PSTH and the FR during motion

overall_cell=0 #number of cells after filtration
effect_cell=0 #number of cells after filtration and with a significant effect between the blocks
    ...
    for learning_task2_inx,(cong_task2,incong_task2) in enumerate(zip([cong_task+'_'+learned_direction,cong_task+'_'+learned_direction+'_100_25_cue'],[incong_task+'_'+learned_direction,incong_task+'_'+learned_direction+'_100_25_cue'])): 
        ...
        for cell_ID in cur_cell_list: #for cells recorded in both motor and passive
                ...
                for block_inx,(begin_inx,end_inx) in enumerate(zip(block_begin_indexes,block_end_indexes)):
                    ...
                    try:
                        dictFilterTrials_cong_test = {'dir':'filterOff', 'trial_name':'v20S', 'fail':0, 'block_begin_end':[begin_inx,end_inx]}
                        dictFilterTrials_incong_test = {'dir':'filterOff', 'trial_name':'v20S', 'fail':0, 'screen_rot':block_base_dir}
                    ...
                    dictFilterTrials_cong_psth = {'dir':'filterOff', 'trial_name':'v20NS', 'fail':0, 'block_begin_end':[begin_inx,end_inx]}
                    dictFilterTrials_incong_psth = {'dir':'filterOff', 'trial_name':'v20NS', 'fail':0, 'screen_rot':block_base_dir}
                    
                    if subtract_BL:
                        FR_cong_BL=np.nanmean(cur_cell_cong.get_mean_FR_event(dictFilterTrials_cong_psth,'motion_onset',window_pre=-800,window_post=-400))
                        FR_incong_BL=np.nanmean(cur_cell_incong.get_mean_FR_event(dictFilterTrials_incong_psth,'motion_onset',window_pre=-800,window_post=-400))
                        ...
                        psth_cong_array.append(cur_cell_cong.PSTH(window_PSTH,dictFilterTrials_cong_psth)-FR_cong_BL)
                        psth_incong_array.append(cur_cell_incong.PSTH(window_PSTH,dictFilterTrials_incong_psth)-FR_incong_BL)
                        FR_cong_array.append(np.nanmean(cur_cong_FR)-FR_cong_BL)
                        FR_incong_array.append(np.nanmean(cur_incong_FR)-FR_incong_BL)
                                                 
                    if cur_alternative=='two-sided':        
                        if np.nanmean(FR_cong)>np.nanmean(FR_incong):
                            FR_cong_array.append(np.nanmean(cur_cell_cong.get_mean_FR_event(dictFilterTrials_cong_psth,'motion_onset',window_pre=100,window_post=300)))
                            FR_incong_array.append(np.nanmean(cur_cell_incong.get_mean_FR_event(dictFilterTrials_incong_psth,'motion_onset',window_pre=100,window_post=300)))
