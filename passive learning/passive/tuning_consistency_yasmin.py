...
#This script shows that cells that change their FR in 100:300 between washout and congruent learning are not consistently tuned.
...
for array_inx,sig_array in enumerate([sig_cong_array,sig_incong_array]):
    ...
    for block_inx,cur_block_dict in enumerate(sig_array):
            ...
            # plt.axhline(mean_washout,linestyle='dotted',color='black')
...
for array_inx,sig_array in enumerate([sig_cong_array,sig_incong_array]):
    ...
    for block_inx,cur_block_dict in enumerate(sig_array):
        try:            
            mapping_base_FR.append(np.nanmean(cur_block_dict['FR mapping base']))
            mapping_learned_FR.append(np.nanmean(cur_block_dict['FR mapping learned']))
            washout_base_FR.append(np.nanmean(cur_block_dict['FR washout test']))
            washout_learned_FR.append(np.nanmean(cur_block_dict['FR washout learned test']))
            n_cells=n_cells+1
        except:
            continue
        
    mapping_tuning=np.array(mapping_base_FR)-np.array(mapping_learned_FR)
    washout_tuning=np.array(washout_base_FR)-np.array(washout_learned_FR)
    res=stats.linregress(mapping_tuning,washout_tuning)
    slope=round(res.slope,3)
    r=round(res.rvalue,3)
    p=round(res.pvalue,4)
    plt.scatter(mapping_tuning,washout_tuning,color=color_array[array_inx])
    plt.axline((0,0),(0,1),color='black')
    plt.axline((0,0),(1,0),color='black')
    plt.title(title_array[array_inx]+ str(n_cells)+' cells '+'slope='+str(round(slope,3))+' r='+str(round(r,3))+' p='+str(round(p,3)))
    plt.xlabel('mapping tuning')
    plt.ylabel('washout tuning')
    plt.show()