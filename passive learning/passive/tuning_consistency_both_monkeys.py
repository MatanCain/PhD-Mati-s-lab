                ...
                if cell_ID<cutoff_cell:
                    ...
                    for block_inx,(begin_inx,end_inx) in enumerate(zip(block_begin_indexes,block_end_indexes)):
                        ...
                        try:
                            ...
                            if p_learning>crit_value_learning:
                                continue
                        except:
                            continue                            
                        ...
                        try:
                            ...
                            #PSTH and FR in learned null direction
                            trialname_null_learned='d'+str(learning_direction_null_int)+trial_type_mapping
                        ...
                        block_dict['PSTH mapping null learned']=psth_mapping_learned
...
#####################################################################################################################################################################                    
                    ...
                    for block_inx,(begin_inx,end_inx) in enumerate(zip(block_begin_indexes,block_end_indexes)):
                        ...
                        #Test if FR during learning block  is different than during washout in critical period            
                        try:
                            dictFilterTrials_learning_test = {'dir':'filterOff', 'trial_name':trial_type_learning, 'fail':0, 'block_begin_end':[begin_inx,end_inx]}
                            dictFilterTrials_washout_test = { 'dir':'filterOff', 'fail':0,'files_begin_end':[file_begin_washout,file_end_washout],'trial_name':'d0'+trial_type_learning}
                        ...
                        block_dict['cell_ID']=cell_ID
                        block_dict['FR learning test']=FR_learning_test           
                        block_dict['FR dishabituation test']=FR_washout_test  
                        block_dict['PSTH mapping base']=psth_mapping_base
                        block_dict['PSTH mapping learned']=psth_mapping_learned
...
for array_inx,sig_array in enumerate([sig_cong_array,sig_incong_array]):
    ...
    mapping_learned_null_psths=np.empty((len(sig_array),len(timecourse)))
    mapping_learned_null_psths[:]=np.nan

    mapping_base_FR=[]
    mapping_learned_FR=[]
    mapping_learned_null_FR=[]
    ...
    for block_inx,cur_block_dict in enumerate(sig_array):
            ...
            if mean_dishabituation> mean_learning:
                ...
                mapping_learned_null_array=cur_block_dict['PSTH mapping null learned']
                ...
                mapping_learned_null_FR.append(np.nanmean(cur_block_dict['FR mapping null learned']))
            ...
            mapping_learned_null_psths[block_inx,:]=mapping_learned_null_array

    #check wether FR in 100-300 between mapping base and learning is significantly different         
    stat,p_learned_base=stats.wilcoxon(mapping_learned_FR,mapping_learned_null_FR)
    p_learned_base=round(p_learned_base,3)
       
    plt.plot(timecourse,np.nanmean(mapping_learned_psths,axis=0),color=color_array[array_inx],linestyle='dotted')
    plt.plot(timecourse,np.nanmean(mapping_learned_null_psths,axis=0),color=color_array[array_inx],linestyle='dashed')
    plt.axvline(x=100,color='black')
    plt.axvline(x=300,color='black')
    plt.legend(['mapping learned','mapping learned null'], loc ="lower right",fontsize=legend_size)
    plt.title(title_array[array_inx]+' '+str(n_cells)+' cells ,p='+str(p_learned_base))
    plt.xlabel('time from MO')
    plt.ylabel('FR')
    plt.show()
    
    plt.scatter(mapping_learned_null_FR,mapping_learned_FR,color=color_array[array_inx])
    plt.axline((0,0),(1,1),color='black')
    plt.title(title_array[array_inx]+' '+str(n_cells)+' cells ,p='+str(p_learned_base))
    plt.xlabel('FR mapping learned null')
