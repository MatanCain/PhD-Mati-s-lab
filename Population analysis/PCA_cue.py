...
condition_array=['saccade','active','passive','cue active','cue passive']
# task_array=['8dir_active_passive_interleaved_100_25','8dir_active_passive_interleaved_100_25']
# trial_type_array=['v20a','v20p']
# event_array=['cue_onset','cue_onset']
# condition_array=['cue active','cue passive']
...
for cond_inx in np.arange(len(task_array)):
    for cond_inx2 in np.arange(len(task_array)):    
        ...
        data_proj_PC_8dir=np.reshape(data_proj_PC,(len(directions),-1))
        #plot the figure        
        ax[cond_inx,cond_inx2].plot(timecourse,np.transpose(data_proj_PC_8dir),linewidth=0.75)
...
# savemat(save_path+"alignement_index_NoLateCueCells"+ ".mat", mdic)

#adjust the directions (the sign of the direction) of the PC such they are consistent across conditions
#switch PC2 direction in saccade
PCs_array[0,1,:]=-PCs_array[0,1,:]
#switch PC1 and PC2 direction in pursuit
PCs_array[1,0,:]=-PCs_array[1,0,:]
PCs_array[1,1,:]=-PCs_array[1,1,:]
#switch PC1 direction in pursuit
PCs_array[1,0,:]=-PCs_array[2,0,:]
#switch PC1 and PC2 direction in cue
PCs_array[3,0,:]=-PCs_array[3,0,:]
PCs_array[3,1,:]=-PCs_array[3,1,:]
